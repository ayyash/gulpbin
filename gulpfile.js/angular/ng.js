...
const ngConfig = {
  Templates: {
    ...
    State: __dirname + angularTemplates + 'state.template',
...
const setConfig = (config) => {
  ...
  const angularTemplates = options.ng.templates || '/angulartemplates/';
...
const _addComponentToModule = function () {
  ...
  // take the first, not the last
  // const majorName = major.substring(major.lastIndexOf('/') + 1);
  const majorName = major.split('/')[0];
  const subMajorName = major.substring(major.lastIndexOf('/') + 1); // could be the same
  // if common or layouts, do not create module
  if (majorName === 'Common' || majorName === 'Layouts') {
    return gulp.src('.');
  }
  ...
  const route =
    fs
      ...
      .replace('major', subMajorName.toLowerCase())
      .replace('name', name.toLowerCase())
      .replace('Major', subMajorName)
      .replace('Name', name);
...
const _createComponent = function () {
  ...
  let _selector = `selector: '${prefix}${majorName ? '-' + majorName.toLowerCase() : ''}-${name.toLowerCase()}',`;
  if (ispartial) {
    _letstandaoen = true;
...
const _createModel = function () {
  ...
  let dest = ngConfig.Destinations.Models + majorName;
  if (majorName === serviceName) {
    dest = ngConfig.Destinations.Models;
  }
  if (!name) {
    return gulp.src('.');
  }
  ...
  return gulp
    ...
    .pipe(replace('_Name_', serviceName))
    .pipe(replace('_name_', serviceName.toLowerCase()))
    .pipe(
      rename({
    ...
    .pipe(gulp.dest(dest, { overwrite: false }));
};
...
const _createService = function () {
  ...
  const majorName = name.split('/')[0];
  const serviceName = name.substring(name.lastIndexOf('/') + 1);
  let dest = ngConfig.Destinations.Services + majorName;
  if (majorName === serviceName) {
    dest = ngConfig.Destinations.Services;
  }

  return gulp
    .src(ngConfig.Templates.Service)
    .pipe(replace('_Name_', serviceName))
    // limitation: when in sub folder, the config.API.name is not defined
    .pipe(replace('_name_', serviceName.toLowerCase()))
    .pipe(
      rename({
        basename: serviceName.toLowerCase(),
...
const _createState = function () {

  const { name, withstate } = params;
  if (!withstate) {
    return gulp.src('.');
  }
  const majorName = name.split('/')[0];
...
module.exports = (config) => {
  ...
  ret.createFullService = gulp.parallel(_createModel, _createService, _addToConfig, _createState);
